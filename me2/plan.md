...
### Task 5 — Real-world demo (individual) — 🔴 not started

Build `demo_pi.py` (new file):
1. **Audio capture** — `sounddevice`/`pyaudio` at 16 kHz from USB mic.
2. **VAD** — energy-based or `webrtcvad` (tiny, on-device) to segment utterances;
...
2. **[me]** Remove pad-to-400 from `demo`/`test` inference paths (issue #5); re-measure
   latency on true lengths.
3. **[me]** Write `requirements.txt` (torch, torchaudio, numpy, sounddevice, webrtcvad).
4. **[you + group]** Start real recording (task 1) — this is the long pole; the synthetic
   generator is only a pipeline stand-in.
5. **[me]** Once ≥ first batch of real recordings exists: build manifest, train 30–50
   epochs, export TorchScript/ONNX.
6. **[me]** Draft the benchmark spec (task 3) for the group's collective review.
7. **[optional]** Resolve HPC key auth (register `SHA256:VjlyfnD45JafiXns+FoOKLE9rbvkptDXpck2gtlkoNo`)
...
- [ ] `demo_pi.py` — mic → VAD → VCM → action dispatch on Pi 5
